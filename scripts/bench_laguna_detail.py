...
async def main():
    ...
    for i, (expect, q) in enumerate(BENCH_100, 1):
        ...
        if status == 429:
            ...
            got, status, dt = await judge_once(q)
        results.append((expect, got, status, dt, feats))
    ...
    # ---- 汇总 ----
    total = len(results)
    ok_all = sum(1 for e, g, s, _, _ in results if s == 200 and g == e)
    rate_200 = sum(1 for e, g, s, _, _ in results if s == 200 and g == e)
    n_429 = sum(1 for _, _, s, _, _ in results if s == 429)
    n_other_err = sum(1 for _, _, s, _, _ in results if s not in (200, 429))
    n_200 = sum(1 for _, _, s, _, _ in results if s == 200)
    ok_200 = sum(1 for e, g, s, _, _ in results if s == 200 and g == e)
    ...
    # 长文本 vs 短文本
    long_ok = sum(1 for e, g, s, _, f in results if s == 200 and g == e and f["long"])
    long_n = sum(1 for _, _, s, _, f in results if s == 200 and f["long"])
    short_ok = sum(1 for e, g, s, _, f in results if s == 200 and g == e and not f["long"])
    short_n = sum(1 for _, _, s, _, f in results if s == 200 and not f["long"])
    print(f"\n长文本(>=40字): {long_ok}/{long_n} ({long_ok / max(1, long_n):.1%})")
    print(f"短文本(<40字): {short_ok}/{short_n} ({short_ok / max(1, short_n):.1%})")

    # 多问题 vs 单问题
    multi_ok = sum(1 for e, g, s, _, f in results if s == 200 and g == e and f["multi"])
    multi_n = sum(1 for _, _, s, _, f in results if s == 200 and f["multi"])
    ...
    # 200 但判错的样本
    wrong = [(e, g, q, f) for (e, g, s, _, f), (_, _, q) in zip(results, BENCH_100) if s == 200 and g != e]
    if wrong:
        print(f"\n200 但判错 {len(wrong)} 条:")
